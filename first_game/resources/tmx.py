...
class Tileset(object):
    ...
    @classmethod
    def fromxml(cls, base_path, tag, firstgid=None):
        if 'source' in tag.attrib:
            firstgid = int(tag.attrib['firstgid'])
            with open(base_path + tag.attrib['source']) as f:
                tileset = ElementTree.fromstring(f.read())
            return cls.fromxml(base_path, tileset, firstgid)
        ...
        for c in tag.getchildren():
            if c.tag == "image":
                # create a tileset
                tileset.add_image(base_path + c.attrib['source'])
            elif c.tag == 'tile':
...
class TileMap(object):
    ...
    def load(cls, filename, viewport):
        from os import path
        path_fragment = path.dirname(filename) + '/'
        ...
        for tag in map.findall('tileset'):
            tilemap.tilesets.add(Tileset.fromxml(path_fragment, tag))
